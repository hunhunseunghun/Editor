...
export async function POST(
  ...
  try {
    ...
    if (documentId.startsWith('temp_')) {
      return NextResponse.json(
        { success: false, error: '임시 문서는 복원할 수 없습니다.' },
        { status: 400 },
      );
    }
    ...
    // 문서 복원 (isDeleted: false, isHiddenFromTrash: false로 설정)
    ...
    return NextResponse.json({
      success: true,
      message: '문서가 성공적으로 복원되었습니다.',
    });
  } catch (error) {
    console.error('문서 복원 에러:', error);
    return NextResponse.json(
      { error: '문서를 복원하는 중 오류가 발생했습니다.' },
      { status: 500 },
    );
  }
}
